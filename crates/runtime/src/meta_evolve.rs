...
impl RuntimeSpec {
    ...
    pub fn get_executor(&self, type_name: &str) -> Option<&CustomExecutorSpec> {
        self.custom_executors.iter().find(|e| e.type_name == type_name)
    }
...
impl ExecutorRegistry {
    ...
    pub async fn custom_executor_types(&self) -> Vec<String> {
        let s = self.spec.read().await;
        s.custom_executors.iter().map(|e| e.type_name.clone()).collect()
    }
    ...
    pub async fn execute(
        ...
        match language.as_str() {
            "python" | "py" => {
                self.execute_script(type_name, "python3", "py", &executor_code, &input_str, timeout_secs, true).await
            }
            "node" | "js" | "javascript" => {
                self.execute_script(type_name, "node", "js", &executor_code, &input_str, timeout_secs, false).await
            }
            "rust" | "wasm" => {
                self.execute_wasm(type_name, &executor_code, &input_str, timeout_secs).await
            }
            "rust_native" | "native" => {
                self.execute_native(type_name, &executor_code, &input_str, timeout_secs).await
            }
    ...
    async fn execute_wasm(
        ...
        let cache_dir = self.storage_path.parent()
    ...
    async fn execute_native(
        ...
        // 5. 注册到缓存（热替换旧版本）
        {
            ...
            if let Some(old) = plugins.insert(type_name.to_string(), plugin) {
                tracing::info!("热替换原生插件: {} (旧 hash: {})", type_name, &old.hash[..8]);
            }
        }
...
pub fn render_node_code(code: &str, input_json: &str) -> String {
    let escaped = input_json.replace('\\', "\\\\").replace('`', "\\`").replace('$', "\\$");

    if code.contains("__EXECUTOR_INPUT__") {
        format!("const __EXECUTOR_INPUT__ = JSON.parse(`{}`);\n{}", escaped, code)
    } else {
        format!(
...
impl NativePlugin {
    ...
    fn call(&self, input: &str) -> Result<serde_json::Value, String> {
        ...
        let input_cstr = CString::new(input)
            .map_err(|e| format!("输入包含 null 字节: {}", e))?;
...
impl MetaEvolver {
    ...
    pub async fn meta_evolve_once(
        ...
        if let Some(proposal) = &report.executor_proposal {
            println!("  🧬 元创造: 提议新执行器 '{}' — {}", proposal.type_name, proposal.description);
            ...
            if test_result {
                ...
                actions.push(format!("元创造: 执行器 '{}' (测试通过)", proposal.type_name));
                ...
                println!("  ❌ 元创造失败: 执行器 '{}' 测试未通过", proposal.type_name);
                actions.push(format!("元创造: 执行器 '{}' (测试失败)", proposal.type_name));
            }
        }
        ...
        for type_name in &custom_types {
            ...
            if self.should_mutate_executor(type_name, evolution).await {
                if let Some(new_code) = self.generate_executor_mutation(type_name).await {
                    match self.registry.mutate_executor(type_name, new_code.0, Some(new_code.1)).await {
                        Ok(_) => {
            ...
            for action in &g.actions {
                let type_name = match &action.implementation {
                    ...
                    ActionImpl::Custom { executor_type, .. } => {
                        type_counts.entry(format!("Custom:{}", executor_type)).or_insert(0);
                        type_counts.entry(format!("Custom:{}", executor_type)).and_modify(|c| *c += 1);
                        continue;
                    }
                };
...
如果无瓶颈，bottleneck_description 设为空字符串，executor_proposal 设为 null。"#,
            ...
            caps_summary = if genomes.is_empty() {
                ...
                genomes.iter()
                    .take(20)
                    .map(|g| {
                        let impl_types: Vec<String> = g.actions.iter()
            ...
            tools = {
                let mut t: Vec<String> = self.platform.env.iter()
        ...
        let v: serde_json::Value = match serde_json::from_str(json_str) {
            Ok(v) => v,
            Err(e) => {
                tracing::warn!("元自省 JSON 解析失败: {} | 原始前200字符: {}", e, safe_truncate(&result, 200));
                return MetaIntrospectionReport::default();
            }
        };

        let bottleneck = v.get("bottleneck_description")
            .and_then(|s| s.as_str())
            .unwrap_or("")
            .to_string();

        let proposal = v.get("executor_proposal")
            .filter(|p| !p.is_null())
    ...
    /// 检查执行器是否需要变异
    async fn should_mutate_executor(
        &self,
        type_name: &str,
        evolution: &EvolutionEngine,
    ) -> bool {
        // 检查使用此执行器的能力是否有低成功率的
        let weak: Vec<_> = evolution.genomes().values()
            .filter(|g| {
                g.actions.iter().any(|a| {
                    if let ActionImpl::Custom { executor_type, .. } = &a.implementation {
                        executor_type == type_name && g.fitness.success_rate < 0.5 && g.fitness.call_count > 2
                    } else {
                        false
                    }
                })
...
{{
  ...
  "new_description": "更新后的描述"
        ...
        let new_desc = v.get("new_description").and_then(|s| s.as_str())?.to_string();
    ...
    async fn find_unused_executors(&self, evolution: &EvolutionEngine) -> Vec<String> {
        let custom_types = self.registry.custom_executor_types().await;
        let used_types: std::collections::HashSet<String> = evolution.genomes().values()
    ...
    async fn count_custom_executor_usage(&self, evolution: &EvolutionEngine) -> u32 {
        evolution.genomes().values()
            .filter(|g| {
                g.actions.iter().any(|a| {
                    matches!(a.implementation, ActionImpl::Custom { .. })
                })
            })
            .count() as u32
    }
...
fn impl_type_name(impl_: &crate::genome::ActionImpl) -> String {
    match impl_ {
        ...
        crate::genome::ActionImpl::Custom { executor_type, .. } => format!("Custom:{}", executor_type),
    }
}
