...
impl AutoEvolver {
    ...
    pub async fn evolve_once(
        ...
        if report.total_capabilities == 0 {
            ...
            // 2. 归因 + 变异：对每个弱能力分析原因并尝试改进（每轮最多3个，避免太慢）
            for weak in report.weak_capabilities.iter().take(3) {
            ...
            // 4.5 自测试：对所有从未被调用过的能力执行自动测试
            //
            // 注意：自测试只证明能力"能跑通"，不能证明能力"有用"。
            // 自测试调用计入 auto_test_count，不计入 real_call_count，
            // 这样自测试通过的能力如果没有真实业务调用，仍会被淘汰。
            let untested: Vec<String> = evolution.genomes().iter()
                .filter(|(_, g)| g.fitness.call_count == 0)
                .map(|(name, _)| name.clone())
                .collect();
            if !untested.is_empty() {
                println!("  🧪 自测试: {} 个能力待测试", untested.len());
            }
            for name in untested.iter().take(5) {
                let pass = self.test_capability(evolution, name).await;
                self.stats.auto_tests += 1;
                if let Some(g) = evolution.genomes_mut().get_mut(name) {
                    // 用 record_auto_test 替代手动重算
                    // 自测试只给低基础分，不清零 rounds_dormant
...
{{
  ...
  "missing": "如果未达成，还缺什么能力或改进"
        ...
        let result = self.llm.execute(&prompt, "auto", None).await;
        match result {
            Ok(text) => {
...
- fix_prompt: {{ mutation_type, capability, action, new_prompt, expected_improvement }}"#,
    ...
    pub async fn apply_mutation(
        ...
        match (plan, &mut action.implementation) {
            (MutationPlan::FixScript { new_code, .. }, ActionImpl::Script { code, .. }) => {
                *code = new_code.clone();
            }
    ...
    pub async fn test_capability(
        ...
        match result {
            Ok(resp) => {
                // 协议约定：响应必须包含 success 字段
                // 没有 success 字段说明能力实现违反协议，视为失败（而非静默通过）
...
返回严格 JSON（直接可用的输入对象，不要包裹在其他结构中）:"#);
        ...
        match self.llm.execute(&prompt, "auto", None).await {
            Ok(text) => {
                let json_str = extract_json(&text);
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(&json_str) {
                    if v.is_object() {
